...
(case when h.TAB_NAME = 'CreateNBS' then 'NBS' when h.TAB_NAME = 'ODSC_Anchor' then 'ODSC' when h.TAB_NAME = 'New_Tenency' then 'New_Tenency' else h.ExtensionType end) as Loading_Type,
...
// 	if($filterStartDate != "")
// 		$sql .= " and h.SR_DATE >= '$filterStartDate' ";
// 	else
// 		$sql .= " and h.SR_DATE is not null ";
// }
// else if($filterSrStatus =="SP"){
// 	if($filterStartDate != "")
// 		$sql .= " and h.SP_DATE >= '$filterStartDate' ";
// 	else
// 		$sql .= " and h.SP_DATE is not null ";
// }
// else if($filterSrStatus =="SO"){
// 	if($filterStartDate != "")
// 		$sql .= " and h.SO_DATE >= '$filterStartDate' ";
// 	else
// 		$sql .= " and h.SO_DATE is not null ";
// }
// else if($filterSrStatus =="RFI"){
// 	if($filterStartDate != "")
// 		$sql .= " and h.RFI_DATE >= '$filterStartDate' ";
// 	else
// 		$sql .= " and h.RFI_DATE is not null ";
// }
// else if($filterSrStatus =="RFI Accepted"){
// 	if($filterStartDate != "")
// 		$sql .= " and h.RFI_ACCEPTED_DATE >= '$filterStartDate' ";
// 	else
// 		$sql .= " and h.RFI_ACCEPTED_DATE is not null ";
// }
...
fputcsv($output, array('SR_Number', 'SP_Number', 'SO_Number', 'Circle_Name', 'TVI_Site_Id', 'Site_Id', 'Site_Name', 'Loading_Type', 'Latitude', 'Longitude', 'Operator', 'Product_Type', 'SR_Raise_Date', 'SP_Raise_Date', 'SO_Raise_Date', 'RFI_Date', 'RFI_Accepted_Date', 'RFS_Date', 'Pending_For'));
...
if($tabName == "dashbord"){
	...
	while($row=mysqli_fetch_assoc($result)){
		...
		if(mysqli_num_rows($approvalFlowResult) !=0){
			...
			'col18' => $row["TVI_SITE_ID"], 'col4' => $row["Site_Id"], 
			'col5' => $row["Site_Name"], 'col19' => $row["Loading_Type"], 'col6' => $row["Latitude"], 'col7' => $row["Longitude"], 'col8' => $row["Operator"], 'col9' => $row["Product_Type"], 
			'col10' => $row["SR_Raise_Date"], 'col11' => $row["SP_Raise_Date"], 'col12' => $row["SO_Raise_Date"], 'col13' => $row["RFI_Date"], 
			'col14' => $row["RFI_Accepted_Date"], 'col15' => $row["RFS_Date"], 'col17' => $row["Pending_For"]);
