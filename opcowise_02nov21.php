...
$airtelOpClause = " SELECT 'Airtel' as OperatorName,a.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` a where a.`Site ID` is  not null and a.`airtel_Tenancy` != 0  and a.`Operational Status` != 'ZTS'";
...
$bsnlOpClause = " SELECT 'BSNL' as OperatorName,b.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` b where b.`Site ID` is  not null and b.`bsnl_Tenancy` != 0  and b.`Operational Status` != 'ZTS'";					
...
$rjioOpClause = " SELECT 'RJIO' as OperatorName,rj.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` rj where rj.`Site ID` is  not null and rj.`rjio_Tenancy` != 0  and rj.`Operational Status` != 'ZTS'";		
...
$tgOpClause = " SELECT 'TTSL-GSM' as OperatorName,tg.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` tg where tg.`Site ID` is  not null and tg.`ttsl_Tenancy` != 0  and tg.`Operational Status` != 'ZTS'";		

$tcOpClause = " SELECT 'TTSL-CDMA' as OperatorName,tc.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` tc where tc.`Site ID` is  not null and tc.`ttslCdma_Tenancy` != 0  and tc.`Operational Status` != 'ZTS'";		
...
$ideaOpClause = " SELECT 'IDEA' as OperatorName,i.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` i where i.`Site ID` is  not null and i.`idea_Tenancy` != 0  and i.`Operational Status` != 'ZTS'";	
...
$vodafoneOpClause = " SELECT 'Vodafone' as OperatorName,v.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` v where v.`Site ID` is  not null and v.`vodafone_Tenancy` != 0 and v.`Operational Status` != 'ZTS'";			
...
$twOpClause = " SELECT 'TCL-Wimax' as OperatorName,tw.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` tw where tw.`Site ID` is  not null and tw.`tclWimax_Tenancy` != 0  and tw.`Operational Status` != 'ZTS'";
...
$tnOpClause = " SELECT 'TCL-NLD' as OperatorName,tn.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` tn where tn.`Site ID` is  not null and tn.`tclNld_Tenancy` != 0 and tn.`Operational Status` != 'ZTS'";	
...
$tiOpClause = " SELECT 'TCL-IOT' as OperatorName,ti.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` ti where ti.`Site ID` is  not null and ti.`tclIot_Tenancy` != 0 and ti.`Operational Status` != 'ZTS'";			
...
$trOpClause = " SELECT 'TCL-Redwin' as OperatorName,tr.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` tr where tr.`Site ID` is  not null and tr.`tclRedwin_Tenancy` != 0  and tr.`Operational Status` != 'ZTS'";	
...
$sifyOpClause = " SELECT 'SIFY' as OperatorName,s.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` s where s.`Site ID` is  not null and s.`sify_Tenancy` != 0  and s.`Operational Status` != 'ZTS'";						
...
$pbOpClause = " SELECT 'PB(HFCL)' as OperatorName,pb.`Circle Name` as CircleName,
					...
					FROM 
					`P&L Sitewise` pb where pb.`Site ID` is  not null and pb.`pbHfcl_Tenancy` != 0 and pb.`Operational Status` != 'ZTS'";	
...
$ztsOpClause = " SELECT 'ZTS' as OperatorName,z.`Circle Name` as CircleName,
				...
				'0' as opTenancy
				FROM 
				`P&L Sitewise` z where z.`Site ID` is  not null
				 and z.`Operational Status` = 'ZTS'";
