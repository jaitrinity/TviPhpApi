...
while($row = mysqli_fetch_assoc($result)){
	...
	$pnlSql = "SELECT `Site ID`, `Site Name`, `Circle Name`, `Billing Circle Name`, `Billing State`, 
	`Airtel_Min of Operator on Date ('April'20)` as `Airtel_Min_Date_$period`, `Airtel_Max of Billing stop Date('April'20` as `Airtel_Max_Date_$period`, 
	`Airtel_Sum of Load`, 
	`BSNL_Min of Operator on Date ('April'20)` as `BSNL_Min_Date_$period`, `BSNL_Max of Billing stop Date('April'20` as `BSNL_Max_Date_$period`, 
	`BSNL_Sum of Load`, 
	`RJIO_Min of Operator on Date ('April'20)` as `RJIO_Min_Date_$period`, `RJIO_Max of Billing stop Date('April'20` as `RJIO_Max_Date_$period`, 
	`RJIO_Sum of Load`, 
	(case when `VIL_Min_Date` = date_format(now(),'%Y-%m-%d') then null else `VIL_Min_Date` end) as `VIL_Min_Date_$period`, 
	(case when `VIL_Max_Date` = date_format(now(),'%Y-%m-%d') then null else `VIL_Max_Date` end) as `VIL_Max_Date_$period`, `VIL_Sum of Load`, 
	(case when `Other_Min_Date` = date_format(now(),'%Y-%m-%d') then null else `Other_Min_Date` end) as `Other_Min_Date_$period`, 
	(case when `Other_Max_Date` = date_format(now(),'%Y-%m-%d') then null else `Other_Max_Date` end) as `Other_Max_Date_$period`, `Other_Sum of Load`, 
	`voltage`, 

	`Airtel(BTS Load in KWH)`, `BSNL(BTS Load in KWH)`, `RJIO(BTS Load in KWH)`, `VIL_BTS_Load_In_KWH`, 
	`Other_BTS_Load_In_KWH`, `Total Load BTS in KW(BTS Load in KWH)`, 

	`Airtel(Aircon Load in KW)`, `BSNL(Aircon Load in KW)`, `RJIO(Aircon Load in KW)`, 
	`VIL_Aircon_Load_in_KW`, `Other_Aircon_Load_in_KW`, `Total Aircon Load in KW(Aircon Load)`, 

	`Airtel(BTS+AC Load)`, `BSNL(BTS+AC Load)`, `RJIO(BTS+AC Load)`, `VIL(BTS+AC Load)`, `Other(BTS+AC Load)`, 
	`Total BTS_AC Load`, 

	`Diesel Billing from Date`, `Diesel Billing Upto Date`, `Diesel No of Days`, `Diesel Cost`, `EB Cost`, `Energy Cost`, 

	`Airtel_Days`, `BSNL_Days`, `RJIO_Days`, `VIL_Days`, `Other_Days`,

	`Airtel(%)`, `BSNL(%)`, `RJIO(%)`, `VIL(%)`, `Other(%)`, `Total %`, 

	`Airtel(Diesel Cost)`, `BSNL(Diesel Cost)`, `RJIO(Diesel Cost)`, `VIL(Diesel Cost)`, `Other(Diesel Cost)`, `ZTS Diesel Cost`, 
	`Total Diesel Cost Share`, 

	`Airtel(EB Cost)`, `BSNL(EB Cost)`, `RJIO(EB Cost)`, `VIL(EB Cost)`, `Other(EB Cost)`, `ZTS EB Cost`, `Total EB Cost`, 

	`Airtel(Energy Cost)`, `BSNL(Energy Cost)`, `RJIO(Energy Cost)`, `VIL(Energy Cost)`, `Other(Energy Cost)`, `ZTS Energy Cost`, 
	`Total Energy Cost Share`, 

	`Airtel(Diesel Revenue)`, `BSNL(Diesel Revenue)`, `RJIO(Diesel Revenue)`, `VIL(Diesel Revenue)`, `Other(Diesel Revenue)`, 
	`ZTS Diesel Revenue`, `Total Diesel Revenue`, 

	`Airtel(EB Revenue)`, `BSNL(EB Revenue)`, `RJIO(EB Revenue)`, `VIL(EB Revenue)`, `Other(EB Revenue)`, `ZTS EB Revenue`, 
	`Total EB Revenue Share`, 

	 `Airtel(Energy Revenue)`, `BSNL(Energy Revenue)`, `RJIO(Energy Revenue)`, `VIL(Energy Revenue)`, 
	 `Other(Energy Revenue)`, `ZTS Energy Revenue`, `Total Energy Revenue Share`, 

	 `Airtel(Margin)`, `BSNL(Margin)`, `RJIO(Margin)`, `VIL(Margin)`, `Other(Margin)`, `ZTS Margin`, `Total Margin Share`, 

	 `No of Tenancy`, 

	 `Operational Status`, `Site Category`, `Site Type`, `FEM`, `NON-FEM`, `EB Status` from `P&L Sitewise` where `UploadMonth` = '$period' ";

	$pnlResult = mysqli_query($conn,$pnlSql);
	$pnlRow=mysqli_fetch_assoc($pnlResult);
	$columnName = array();
	foreach ($pnlRow as $key => $value) {
	...
	mysqli_data_seek($pnlResult, 0);
	while($pnlRow=mysqli_fetch_assoc($pnlResult)){
		$exportData = array();
		foreach ($columnName as $key => $value) {
			array_push($exportData, $pnlRow[$value]);
		}
