...
if($tabName == "Site_Upgrade" && $methodType === 'POST'){
	...
	for($i=0;$i<count($headerData);$i++){
		...
		$tableColumn .= "`CIRCLE_NAME`, `Operator`, `No_Of_Poles`, `No_Of_HPSC_Antenna`, `NO_OF_BBU`, `NO_OF_RRU`, `TAB_NAME`, `SR_DATE`, 
		`STATUS`, `Insert_Type`, `CREATE_BY`, `CREATE_DATE`";
		$tableData .=  "'$circleName', '$operator', $noOfPoles, $noOfHpsc, $noOfBBU, $noOfRRU, '$tabName', '$todayDate', 'NB01', 'ExcelUpload', 
