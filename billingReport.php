...
if($rowcount == 0){
	...
	if(strtotime($firstDate) < strtotime($compDate)){
		$airtel = "SELECT `Circle Name` as telecom_circle, `Billing State` as billing_state, '' as sub_state_name, '' as billing_type, '' as bp_code, '' as billing_nature, 'Airtel' as operator_name, '' as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `From DT` as billing_start_date, `To DT` as billing_end_date, `Site ID` as site_id, `Site Name` as site_name, `Total no of Operators` as tenancy, `Diesel rate` as diesel_rate, `Airtel Diesel Share` as diesel_share, '' as dg_cabex, `EB Bill from Date` as eb_from_date, `EB bill upto Date` as eb_end_date, `No of days` as no_of_days, '' as eb_unit_rate, `Airtel EB Share` as eb_share, '' as eb_cabex, `Airtel P&F Share Share` as total, '' as total_cabex FROM `airtel_provision_consoldata` where `Bill_Month` in ('$periodString') ";
	}
	else{
		if($periodString == 'Apr-21'){
			$airtel = "SELECT `Circle Name` as telecom_circle, `Billing State` as billing_state, '' as sub_state_name, '' as billing_type, '' as bp_code, '' as billing_nature, 'Airtel' as operator_name, '' as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `From DT` as billing_start_date, `To DT` as billing_end_date, `Site ID` as site_id, `Site Name` as site_name, `Total no of Operators` as tenancy, `Diesel rate` as diesel_rate, `Airtel Diesel Share` as diesel_share, '' as dg_cabex, `EB Bill from Date` as eb_from_date, `EB bill upto Date` as eb_end_date, `No of days` as no_of_days, '' as eb_unit_rate, `Airtel EB Share` as eb_share, '' as eb_cabex, `Airtel P&F Share Share` as total, '' as total_cabex FROM `airtel_provision_consoldata` where `Bill_Month` in ('$periodString') 
			Union All
			SELECT `Circle Name` as telecom_circle, `Billing State` as billing_state, `Sub State Name` as sub_state_name, `Billing Type` as billing_type, `BP Code` as bp_code, `Billing nature` as billing_nature, 'Airtel' as operator_name, `Energy type` as enery_type, SUBSTRING(`Period`, 1,3) as billing_month, `From Date` as billing_start_date, `To Date` as billing_end_date, `Site ID` as site_id, `Site Name` as site_name, `Final Tenancy For Billing` as tenancy, `Diesel Rate` as diesel_rate, `Airtel Diesel Amount` as diesel_share, '' as dg_cabex, `From Date` as eb_from_date, `To Date` as eb_end_date, `Days` as no_of_days, `EB Rate` as eb_unit_rate, `Airtel EB Amount` as eb_share, '' as eb_cabex, `Diesel+EB` as total, '' as total_cabex FROM `airtel_output_april` where `Period` in ('$periodString') and `Billing nature` is null ";
		}
		else{
			$airtel = "SELECT `Circle Name` as telecom_circle, `Billing State` as billing_state, '' as sub_state_name, '' as billing_type, '' as bp_code, '' as billing_nature, 'Airtel' as operator_name, '' as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `From DT` as billing_start_date, `To DT` as billing_end_date, `Site ID` as site_id, `Site Name` as site_name, `Total no of Operators` as tenancy, `Diesel rate` as diesel_rate, `Airtel Diesel Share` as diesel_share, '' as dg_cabex, `EB Bill from Date` as eb_from_date, `EB bill upto Date` as eb_end_date, `No of days` as no_of_days, '' as eb_unit_rate, `Airtel EB Share` as eb_share, '' as eb_cabex, `Airtel P&F Share Share` as total, '' as total_cabex FROM `airtel_provision_consoldata` where `Bill_Month` in ('$periodString') ";
		}
	...
	$bsnl = "SELECT `Circle` as telecom_circle, `Billing State` as billing_state, `Sub State Name` as sub_state_name, `Billing Type` as billing_type, `BP Code` as bp_code, `Billing nature` as billing_nature, 'BSNL' as operator_name, `Energy type` as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `From_Date` as billing_start_date, `To_Date` as billing_end_date, `IP Site ID` as site_id, `Site Name` as site_name, `No_of_Tenancy` as tenancy, `Diese Rate` as diesel_rate, `Total Diesel cost` as diesel_share, '' as dg_cabex, '' as eb_from_date, '' as eb_end_date, `Days` as no_of_days, `EB Rate` as eb_unit_rate, `Total EB cost` as eb_share, '' as eb_cabex, `Total Amount (EB+DG)` as total, '' as total_cabex FROM `bsnl_output_april` where `Bill_Month` in ('$periodString')";

	$pbHfcl = "SELECT `CIRCLE_NAME` as telecom_circle, `Billing State` as billing_state, `Sub State Name` as sub_state_name, `Billing Type` as billing_type, `BP Code` as bp_code, `Billing nature` as billing_nature, 'PB_HFCL' as operator_name, `Energy type` as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `From Date` as billing_start_date, `To Date` as billing_end_date, `IP Site ID` as site_id, `Site Name` as site_name, `No_of_Tenancy` as tenancy, `DG Current Rate` as diesel_rate, `DG Billable Amt` as diesel_share, '' as dg_cabex, '' as eb_from_date, '' as eb_end_date, `Days` as no_of_days, `EB Current Rate` as eb_unit_rate, `EB Billable Amt` as eb_share, '' as eb_cabex, `Final Billable Amt` as total, '' as total_cabex FROM `pbhfcl_output_april` where `Bill_Month` in ('$periodString') ";

	
	$rjio = "";
	if(strtotime($firstDate) < strtotime($compDate)){
		$rjio = "SELECT `Billing Circle` as telecom_circle, `Billing State` as billing_state, `Sub State Name` as sub_state_name, '' as billing_type, `BP Code` as bp_code, `Billing nature` as billing_nature, 'RJIO' as operator_name, `Energy type` as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `EffectiveStartDate` as billing_start_date, `EffectiveEndDate` as billing_end_date, `TVI Site ID` as site_id, `SITE NAME` as site_name, `No of Tenants` as tenancy, `DGRate` as diesel_rate, (`DG_CPH_Cabex` * `DGRate` * `DGHrs` * `NoOfDays` + `DG_Billing_Amt`) as diesel_share, `Cabex_DG` as dg_cabex, `EffectiveStartDate` as eb_from_date, `EffectiveEndDate` as eb_end_date, `NoOfDays` as no_of_days, `EBRate` as eb_unit_rate, (`EB_CPH_Cabex` * `EBRate` * `EBHrs` * `NoOfDays` + `EB_Billing_Amt`) as eb_share, `Cabex_EB` as eb_cabex, `TotalAmountChargable` as total, (`Cabex_DG` + `Cabex_EB`) as total_cabex FROM `rjio_output_april` where `Bill_Month` in ('$periodString') ";
	}
	else{
		$rjio = "SELECT `Billing Circle` as telecom_circle, `Billing State` as billing_state, `Sub State Name` as sub_state_name, '' as billing_type, `BP Code` as bp_code, `Billing nature` as billing_nature, 'RJIO' as operator_name, `Energy type` as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `EffectiveStartDate` as billing_start_date, `EffectiveEndDate` as billing_end_date, `TVI Site ID` as site_id, `SITE NAME` as site_name, `No of Tenants` as tenancy, `DGRate` as diesel_rate, (`Cabex_DG` + `DG_Billing_Amt`) as diesel_share, `Cabex_DG` as dg_cabex, '' as eb_from_date, '' as eb_end_date, `NoOfDays` as no_of_days, `EBRate` as eb_unit_rate, (`Cabex_EB` + `EB_Billing_Amt`) as eb_share, `Cabex_EB` as eb_cabex, `TotalAmountChargable` as total, (`Cabex_DG` + `Cabex_EB`) as total_cabex FROM `rjio_output_april` where `Bill_Month` in ('$periodString') ";
	}

	$tcl = "SELECT `Circle Name` as telecom_circle, `Billing State` as billing_state, `Sub State Name` as sub_state_name, '' as billing_type, `BP Code` as bp_code, `Billing nature` as billing_nature, `CustomerName` as operator_name, `Energy type` as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `From Date` as billing_start_date, `To Date` as billing_end_date, `IP Site ID` as site_id, `Site Name` as site_name, `No_of_Tenancy` as tenancy, `DG Current Rate` as diesel_rate, `DG Billable Amt` as diesel_share, '' as dg_cabex, '' as eb_from_date, '' as eb_end_date, '' as no_of_days, `EB Current Rate` as eb_unit_rate, `EB Billable Amt` as eb_share, '' as eb_cabex, `Final Billable Amt` as total, '' as total_cabex FROM `tcl_output_april` where `Bill_Month` in ('$periodString')";

	$ttsl = "SELECT `Circle` as telecom_circle, `Billing State` as billing_state, `Sub State Name` as sub_state_name, '' as billing_type, `BP Code` as bp_code, `Billing nature` as billing_nature, 'TTSL' as operator_name, `Energy type` as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `Start Date` as billing_start_date, `End Date` as billing_end_date, `SITE ID` as site_id, `SITE NAME` as site_name, `Total no of Operators` as tenancy, `Diesel rate` as diesel_rate, `Total Diesel Share` as diesel_share, '' as dg_cabex, `From DT` as eb_from_date, `To DT` as eb_end_date, `No of days` as no_of_days, '' as eb_unit_rate, `Total EB Share` as eb_share, '' as eb_cabex, `Total P&F Share` as total, '' as total_cabex FROM `ttsl_output_april` where `Bill_Month` in ('$periodString') ";

	$vil = "SELECT `Telecom Circle Name` as telecom_circle, `GST State` as billing_state, '' as sub_state_name, '' as billing_type, `BP Code` as bp_code, '' as billing_nature, `Operator Name` as operator_name, '' as enery_type, SUBSTRING(`Bill_Month`, 1,3) as billing_month, `Start Date` as billing_start_date, `End Date` as billing_end_date, `TVI Site ID` as site_id, `TVI Site Name` as site_name, `No of Operational Tenancy` as tenancy, '' as diesel_rate, `VIL Diesel Share` as diesel_share, '' as dg_cabex, `EB Bill from Date` as eb_from_date, `EB bill upto Date` as eb_end_date, `EBBill_No of days` as no_of_days, '' as eb_unit_rate, `VIL EB Share` as eb_share, '' as eb_cabex, `VIL Total Energy Share` as total, '' as total_cabex FROM `vil_output_consol` where `Bill_Month` in ('$periodString') ";
